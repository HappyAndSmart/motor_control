...
uint8_t bmi088_config_init(void)
{
...
void BMI088_gyro_read_over(uint8_t *rx_buf, float gyro[3], float gy_offset[3])
{
    int16_t bmi088_raw_temp;
    bmi088_raw_temp = (int16_t)((rx_buf[1]) << 8) | rx_buf[0];
    gyro[0] = bmi088_raw_temp * BMI088_GYRO_SEN - gy_offset[0];
    bmi088_raw_temp = (int16_t)((rx_buf[3]) << 8) | rx_buf[2];
    gyro[1] = bmi088_raw_temp * BMI088_GYRO_SEN - gy_offset[1];
    bmi088_raw_temp = (int16_t)((rx_buf[5]) << 8) | rx_buf[4];
    gyro[2] = bmi088_raw_temp * BMI088_GYRO_SEN - gy_offset[2];
}

void BMI088_read(float gyro[3], float accel[3], float *temperate, float *gy_offset)
{
